...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  double * normal;
  for (i = 0; i < polygons->lastcol - 2; i=i+3) {
...
/*======== void add_sphere() ==========
  ...
  Since polygons are drawn using 2 points, add each point twice,
...
void add_sphere( struct matrix * polygons,
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      a = index;
      b = (index+1)%points->lastcol;
      c = (index+steps+1)%points->lastcol;
      d = (index+steps)%points->lastcol;

      if(longt != steps-2){
          add_polygon(polygons, points->m[0][a], points->m[1][a], points->m[2][a],
                                points->m[0][b], points->m[1][b], points->m[2][b],
                                points->m[0][c], points->m[1][c], points->m[2][c]);
      }
      if(longt != 0){
          add_polygon(polygons, points->m[0][a], points->m[1][a], points->m[2][a],
                                points->m[0][c], points->m[1][c], points->m[2][c],
                                points->m[0][d], points->m[1][d], points->m[2][d]);
      }
    }
  }
  free_matrix(points);
  ...
  void add_torus( struct matrix * edges,
    ...
    for ( lat = latStart; lat < latStop; lat++ ) {
      for ( longt = longStart; longt < longStop; longt++ ) {
        ...
        if(longt != steps-2){
          add_polygon(edges,points->m[0][a],points->m[1][a],points->m[2][a]
                           ,points->m[0][b],points->m[1][b],points->m[2][b]
                           ,points->m[0][c],points->m[1][c],points->m[2][c]);
        }
        if(longt != 0){
